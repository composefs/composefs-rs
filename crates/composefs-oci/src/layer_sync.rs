...
pub fn drain_splitdirfdstream_verified<ObjectID: FsVerityHashValue>(
    ...
    while let Some(chunk) = reader.next_chunk().context("splitdirfdstream read error")? {
        match chunk {
            ...
            Chunk::InlineData(data) => {
                // Non-world-readable file transported inline. Hash the bytes
                // directly — we already have them, no fd needed for hashing or
                // for the small-file inline path.
                hasher.update(data);
                let length = data.len() as u64;
                if should_inline(length) {
                    stats.bytes_inlined += length;
            ...
            Chunk::FileBackedData {
                ...
                let actual_size = rustix::fs::fstat(&obj_file)
                    .with_context(|| format!("fstat object {name}"))?
                    .st_size as u64;
                ...
                hash_fd_contents(&obj_file, length, &mut hasher)
                    .with_context(|| format!("hashing object {name}"))?;

                // Store the object.  process_file_content takes an OwnedFd;
                // convert our File back while keeping the underlying fd alive.
                process_file_content(
                    &repo,
                    &mut writer,
                    &mut stats,
