...
All EROFS metadata images within a composefs artifact MUST use EROFS format version 1 (v1). The `v1` in the `application/vnd.composefs.v1.erofs` media type denotes this format version. Producers MUST author v1 EROFS images; consumers MUST reject any artifact whose EROFS metadata is not v1. Pinning to v1 keeps the fsverity digest stable and deterministic across implementations, which is essential for signature verification.
