...
/// Read and parse an EROFS image fd into a [`composefs::tree::FileSystem`].
///
/// The file is read into a temporary buffer; the returned `FileSystem` is
/// fully owned and does not borrow from the fd or the buffer.
pub fn erofs_fd_to_filesystem<ObjectID: FsVerityHashValue>(
    fd: OwnedFd,
) -> anyhow::Result<composefs::generic_tree::FileSystem<composefs::tree::RegularFile<ObjectID>>> {
    use anyhow::Context as _;
    ...
    erofs_to_filesystem::<ObjectID>(&buf).context("parsing EROFS image")
}
