...
/// RAII guard that tears down a `cfsctl mount --fuse` subprocess and its FUSE
...
fn privileged_fuse_dumpfile_roundtrip() -> Result<()> {
    ...
    // 5. Spawn `cfsctl mount --raw-image --fuse` in the background. It opens
    //    /dev/fuse, mounts, attaches at <mountpoint>, and serves until killed.
