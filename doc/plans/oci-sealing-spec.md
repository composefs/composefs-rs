...
See [Composefs Artifact Structure](#composefs-artifact-structure) below for more information about the layout.
...
Signature layers are raw PKCS#7 DER-encoded blobs — exactly the format expected by `FS_IOC_ENABLE_VERITY`. No JSON wrapping or base64 encoding. Prebuilt EROFS layers (when present) are raw EROFS images.
...
**EROFS-alongside example** (prebuilt EROFS on registry):
...
```json
{
  ...
  "layers": [
    {
      "mediaType": "application/vnd.composefs.v1.erofs",
      "digest": "sha256:fff...",
      "size": 8192,
      "annotations": {
...
1. **(erofs-alongside only)** N EROFS metadata entries with `composefs.erofs.type: "layer"` — one per manifest layer, in manifest order. Each is a raw EROFS metadata image.
...
2. Extract EROFS metadata layers from the artifact
...
- `application/vnd.composefs.v1.erofs`: Layer media type for prebuilt EROFS metadata images (erofs-alongside only)
