...
/// Like [`read_filesystem`] but accepts any [`ObjectStore`] implementation.
///
/// This is the preferred entry point when using a custom store (e.g.
/// [`FlatDigestStore`] for C-compatible `--digest-store` behaviour).
pub async fn read_filesystem_with_store<ObjectID: FsVerityHashValue>(
    dirfd: OwnedFd,
    path: PathBuf,
...
pub async fn read_filesystem_with_semaphore<ObjectID: FsVerityHashValue>(
    ...
    let store: Option<Arc<dyn ObjectStore<ObjectID>>> =
        repo.map(|r| r as Arc<dyn ObjectStore<ObjectID>>);
    read_filesystem_impl(dirfd, path, store, Some(semaphore)).await
}
