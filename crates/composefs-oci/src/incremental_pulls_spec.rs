...
//!
//! Composefs changes this picture fundamentally. The locally-generated EROFS metadata contains the complete filesystem tree with fsverity digests for every content object. A composefs-aware client knows which objects it already has, and can compute which ones are missing.
//!
...
//! |--------|-------------|---------|----------------------|
//! | TOC format | JSON in zstd skippable frame | JSON in gzip member | Offset map (separate OCI artifact) |
//! | TOC reuse | Discarded after pull | Discarded after pull | EROFS generated locally; mounted by the kernel |
...
//!
//! Tar entries are in **canonical order**, the same deterministic ordering defined by the [canonical tar format](crate::canonical_tar_spec). This is essential: a client that does an incremental pull must be able to regenerate byte-identical tar for push, so the entry ordering cannot be compression-driven.
//!
...
//!
//! 1. Fetch the composefs metadata artifact (offset map + optional signatures)
//! 2. Read the composefs digest annotations to learn expected fsverity digests for all non-inline content objects
...
//!
//! No tar reassembly, no diff_id verification, no tar-split. Trust is rooted in the composefs digest (signed or verified via the manifest chain), and each content object is independently verified by its fsverity digest. After fetching all missing objects, the client generates the EROFS locally and verifies its fsverity digest matches the expected value.
//!
...
//!
//! This requires a **canonical tar format**: a well-defined, reproducible mapping from filesystem metadata (EROFS or dumpfile) + content objects to a tar byte stream. See [`canonical_tar_spec`](crate::canonical_tar_spec) for this specification.
//!
...
//!
//! The offset map is an additional layer in the composefs metadata artifact (`application/vnd.composefs.metadata.v1`). With incremental pull support, the artifact layers are ordered:
//!
//! 1. N offset map layers (one per image layer, `application/vnd.composefs.v1.offset-map`)
//! 2. *(Optional)* Signature layers (`application/vnd.composefs.signature.v1+pkcs7`)
//!
//! Each offset map layer carries a `composefs.layer.offset-map-index` annotation identifying which manifest layer it corresponds to (0-indexed).
//!
...
//! ## References
//!
//! - [OCI sealing specification](crate::sealing_spec): composefs metadata artifacts
//! - [Canonical tar format](crate::canonical_tar_spec): reproducible tar generation for push after incremental pull
//! - Standardized EROFS metadata (future): canonical EROFS generation (separate concern)
